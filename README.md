...
[![Version](https://img.shields.io/badge/version-3.6-orange.svg)](#-version-history)
...
- [Overview](#-overview)
- [What AnnoMining Does](#-what-annomining-does)
- [New in Version 3.6](#-new-in-version-36)
...
- [Disease Resistance Analysis](#-disease-resistance-analysis)
- [TNJ Gene Detection](#-tnj-gene-detection)
...
AnnoMining identifies and characterizes genes from annotated plant genomes using a **multi-evidence strategy**:
...
This approach maximizes recovery of genes of interest, including species-specific or poorly characterized genes in non-model plants.
...
| Tool | Purpose |
|---|---|
| **gffread** | Extracting proteins from GFF files |
| **HMMER** | Profile searches (`hmmsearch`) |
...
### Step 3 — Install HMMER profiles

```bash
mkdir -p hmmer_profiles
cd hmmer_profiles

# Download Pfam-A database (~400 MB compressed, ~2 GB uncompressed)
...
### Step 4 — Download the AnnoMining script
...
| File | Description |
|---|---|
| `*_combined_high_medium_pfam_dotplot.pdf` | Dot plot of enriched PFAM domains in HIGH+MEDIUM genes |
...
Detailed pharmaceutical potential analysis of secondary metabolism genes.

**Steps:**

1. Select input files — same EggNOG and InterProScan files.
2. Set parameters:
   - **Threshold Score:** default `4` (lower = more MEDIUM genes).
   - **Network Nodes:** number of genes in functional network (default `15`).
   - **Network Method:** `enrichment`, `score`, or `balanced`.
3. Run analysis.

#### 3. Disease Resistance Tab

Comprehensive disease resistance gene analysis.
...
| Issue | Solution |
|---|---|
| `gffread not found` | Install `gffread`: `conda install -c bioconda gffread` |
...
| `No resistance genes detected` | May be biologically accurate for some species. Check that resistance domains are present in your HMM profiles. |
...
> They provide complementary information. EggNOG gives KEGG pathways and KOs, while InterProScan provides PFAM domains. Combining them improves detection.

**Q: What does HMMER add to the analysis?**

> HMMER can detect genes with characteristic domains even if they lack functional annotation in databases. This is crucial for non-model species and for detecting resistance genes that may be species-specific.

**Q: What are TNJ genes and why are they important?**

> TNJ (TIR-NBS-Jacalin) genes are a Myrtaceae-specific family of disease resistance genes. They combine TIR and NB-ARC domains with a Jacalin lectin domain. Detection of these genes can indicate species-specific resistance mechanisms.

**Q: How do I know if HMMER found new genes?**

> The log will show `HMMER identified X genes with domains`. The statistics file will show the total secondary/resistance gene count.

**Q: Can I run AnnoMining without the GUI?**

> Yes. Use command-line mode with the `--mode` argument.

**Q: What if my genome has no EggNOG or InterProScan annotations?**

> AnnoMining requires at least EggNOG annotations. Consider running EggNOG-mapper on your proteome first.

**Q: How long does the analysis take?**

> For a typical plant genome (~30,000 genes), secondary metabolism analysis takes 5–15 minutes. Resistance analysis adds 5–10 minutes. HMMER searches depend on the number of profiles used.

**Q: What does the "Other" category represent?**

> The "Other" category includes pathways that don't fit the main classes. Check the specific pathways in the table.

**Q: Can I add my own HMM profiles?**

> Yes. Add your profiles to the `hmmer_profiles` directory and update the appropriate dictionary in the script.

**Q: Why didn't my genome detect any TNJ genes?**

> TNJ genes are specific to Myrtaceae and may not be present in all species. Additionally, some Myrtaceae species naturally lack TNJ genes. Check that your genome has TIR and NB-ARC domains (these are more broadly distributed).

**Q: How do I interpret the resistance network?**

> The resistance network shows connections between genes based on shared resistance classes, pathways, and PFAM domains. Node color indicates resistance potential (HIGH/MEDIUM/LOW), and 🔬 indicates TNJ genes.
...
> **AnnoMining v3.6: A comprehensive pipeline for secondary metabolism, pharmaceutical potential, and disease resistance discovery in plant genomes.**
